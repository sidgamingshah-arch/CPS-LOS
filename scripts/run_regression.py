...
SUITES = [
    ...
    ("e2e_risk_flags",       "Configurable RISK_FLAG catalogue + CDD/HITL governance (tester hardening): the six historical counterparty risk flags become a config-service RISK_FLAG master (label/screeningListSource/defaultSeverity/defaultScore/matchedAttributes/order + descriptive cddImpact/blocksKyc, byte-identically reproducing the hardcoded flags) -- advisory screening (PEP 0.92/HIGH -> ADVERSE_MEDIA 0.74/MEDIUM -> OFAC 0.68/HIGH -> WORLDCHECK 0.41/LOW, in order) is driven from the catalogue with NO code branch, while CDD tiering (pep->ENHANCED, listedEntity->SIMPLIFIED, none->STANDARD, complexOwnership->ENHANCED) stays authored in the per-jurisdiction CDD_TIERS rule pack (the authoritative, editable source); GOVERNED no-simulated-text -- with no LLM a generated hit's aiRationale is NOT fabricated (null, rationaleSource=NONE) and a named human records one via /screening/{id}/rationale (rationaleSource=HUMAN, encrypted at rest, blank->400); HITL SoD -- verifyKyc blocks the counterparty creator (maker!=checker 403) and a distinct human signs off ->VERIFIED, obligor approval blocks the prospect creator (403) and a distinct human promotes ->OBLIGOR; forward-compatible -- a config-defined flag beyond the six booleans is captured via extraRiskFlags with no schema change"),
    ("e2e_cp_docautofill",   "Counterparty create-screen doc autofill (batch4): counterparty-scoped POST /extract-doc (multipart file OR {text}) deterministically parses trade-licence/MOA/AOA identity fields (legalName/cin/registrationNo/gstin/directors) as ADVISORY suggestions to pre-fill the create form; nothing is persisted by extract (audit.ai advisory=true), a second different document yields different values (no template), the human still edits+submits the real audited create"),
    ("e2e_deal_edit",        "Deal facility/collateral/sublimit EDIT (batch4): PATCH facility/collateral/sublimit + DELETE collateral, pre-sanction-gated (post-sanction -> 409 to the amendment path), primary facility non-editable; sublimit interchangeable-group combined cap = max/shared (NOT the sum) with no fungible double-count in the facility-cap check; collateral market value stays behind the revalue gate (descriptive PATCH only); HUMAN-audited"),
    ("e2e_collateral_upload","Collateral-intel document upload (batch4): POST /collateral-intel/{ref}/extract accepts an uploaded document (documentId -> DMS OCR text) feeding the same advisory extract pipeline; suggested collateral values derive from the document text; the human confirm remains the sole writer of a Collateral; deal envelope + rating byte-identical across the extract"),
    ("e2e_fin_template",     "Financials template-driven extraction (batch4): the extraction-field->canonical-line mapping resolves from the FINANCIAL_TEMPLATE master (payload.extractionMap) merged over the built-in fallback (byte-identical when omitted); a confirmed FINANCIAL_STATEMENT extraction auto-drafts an UNCONFIRMED DRAFT spread (spreadConfirmed=false) only on an EMPTY grid (never clobbers an existing spread), after commit (never fails the confirm); authoritative confirmed-spread/rating byte-identical"),
    ("e2e_capital_facilities","Per-facility capital + dual pricing (batch4): a SINGLE-facility deal yields byte-identical RWA/capital + pricing (legacy path); a MULTI-facility deal aggregates per-facility RWA (facility-type CCF + linked/pooled collateral); pricing returns per-facility RAROC + a deal aggregate; the recommended price shows RAROC + a deterministic PEER_PRICING dual price (fail-soft, never blended); hurdle read from the PRICING pack incl. segment overrides"),
    ("e2e_override_config",  "Override-notch config (batch4): notch ceilings resolve from the OVERRIDE_ROLE master (built-in fallback on outage), enforced from the AUTHENTICATED actor's roles (a request-body role claim is never a grant); editing the master's score moves the enforced limit; ANALYST beyond 1 notch -> 403, CRO large override allowed"),
    ("e2e_proposal_sections","Expanded bank-CAM proposal (batch4): the FULL_CAM PROPOSAL_FORMAT lists+renders the 12 new section builders (background/industry/trend/risks/perfection/conduct/deviations/raroc/esg/conditions/recommendation/compliance); multi-year financial_trend renders >1 period; missing data degrades gracefully; generation leaves authoritative rating/pricing/capital byte-identical"),
    ("e2e_discoverability",  "Discoverability (batch4): GET /syndication/deals returns SYNDICATION-only deal summaries (reference/borrower/totalCommitment/lenderCount) for the syndicated-deal filter; additive read-only, route does not collide with /{reference}/book"),
    ("e2e_rbac_gates",       "RBAC governance gates (round-3 defect sweep): origination is FIRST-LINE — a COMPLIANCE actor is denied (403) both deal-origination and prospect-creation via the soft ProtectedAction.ORIGINATE gate (ActorDirectory.requireRecognised: recognised control-function roles denied, unroled/system actors permissive), while an RM proceeds; CPT sign-off is a named relationship-owner act — an ANALYST is denied (403) via the hard ProtectedAction.CPT_REVIEW gate, an RM confirms; both fixes enforced server-side"),
    ("e2e_committee_ladder", "Stage-4 committee / DoP ladder (R3-2A): the DOA_MATRIX pack's active quantum×grade tiers now carry a NAMED committee + composition (behaviour-preserving — authority/quorum unchanged, asserted by e2e_smoke/e2e_decisioning); the full PSB committee ladder (Regional/Zonal → Circle/FGM → HOCAC-I/II/III → MCB → Board), the cross-cutting escalation matrix and the three lines of defence (RM=FIRST, credit=SECOND, board=OVERSIGHT; compliance=SECOND) are exposed as config-as-data via GET /api/decisions/doa/ladder for IN-RBI + AE-CBUAE"),
    ("e2e_rbac_config",      "RBAC configuration at admin level (R3-2B): the ACTION_ROLE master overlays the compile-time ProtectedAction enum (maker-checker), so who-may-perform-an-action is admin-editable DATA; GET /api/governance/rbac/actions exposes the effective catalogue (seeded byte-identical to the enum, source ACTION_ROLE_MASTER); the override has TEETH — remap `originate` to admit COMPLIANCE + invalidate the cache -> a COMPLIANCE actor can originate (was 403), revert -> blocked again; the enum stays the safe fallback"),
    ("e2e_deal_queue",       "Hybrid deal work-queue (R3-2C): the first (INTAKE) workflow stage now carries a queueKey, so creating a deal mints a WorkItem into the owning role's ASSIGNMENT_POOL queue (origination.queue, auto-assigned rm.user/rm.head); the deal stays globally searchable (hybrid — the WorkItem is a best-effort mirror that never gates the deal); ownership moves only on supervisor reassignment (mandatory reason, SoD)"),
    ("e2e_montecarlo",       "Monte-Carlo financial projections (R3-3A): an advisory ML overlay runs N stochastic iterations of the deterministic per-line proforma with each driver sampled Normal(mean, |mean|×volatility) — volatility calibrated in the PROJECTION_TEMPLATE from industry/peer/historical inputs; reports a well-ordered final-year DSCR band (P10<=P50<=P90) + covenant-breach probability P(DSCR<1) + per-line distributions; deterministic (seeded from the deal ref, a re-run reproduces the same P50); governed by the MONTE_CARLO capability; ADVISORY — the authoritative rating grade is byte-identical before/after and the run is an AI audit event"),
    ("e2e_cad_docai",        "CAD document-AI (R3-3B): advisory signature-specimen match + title/property-document field extraction & completeness check over a CAD checklist item — a CadDocVerification finding (advisory=true) governed by the CAD_DOC_AI capability, audit.ai on the run + audit.human on accept/reject; it NEVER auto-COMPLIES the item (a named human still sets the checklist status); fail-soft deterministic with an optional LLM overlay"),

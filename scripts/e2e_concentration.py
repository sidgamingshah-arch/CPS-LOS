...
# A correlated sector takes a PARTIAL hit — its stress MULTIPLE is below the full 4x shock the
# directly-shocked sector receives. (Comparing the multiple, not cross-sector average PDs, is the
# robust test of "partial, not full": a correlated sector with a higher base PD can still carry a
# higher average stressed PD while its multiple stays below the shock — e.g. once real sectors
# feed the book via D6, the MANUFACTURING bucket's composition varies run to run.)
_mfg = secs["MANUFACTURING"]
_infra = secs["INFRASTRUCTURE"]
_mfg_mult = _mfg["avgStressedPd"] / _mfg["avgBasePd"] if _mfg["avgBasePd"] else 0
_infra_mult = _infra["avgStressedPd"] / _infra["avgBasePd"] if _infra["avgBasePd"] else 0
check("a correlated sector (MANUFACTURING) takes a partial PD hit, not the full shock",
      _mfg["avgBasePd"] < _mfg["avgStressedPd"] and _mfg_mult < _infra_mult,
      f"mfg x{_mfg_mult:.2f} vs infra x{_infra_mult:.2f} (mfg {_mfg['avgBasePd']}->{_mfg['avgStressedPd']})")

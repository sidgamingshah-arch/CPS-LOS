...
python3 scripts/e2e_smoke.py            # full lifecycle, 318 assertions through the gateway
...
| counterparty-service | 8082 | `/counterparty` | KYC/CDD/UBO/screening + **credit-initiation lifecycle** `/api/initiation` (prospect→obligor, dedup, negative check, source-system check façade, RM ownership, groups, **AI group identification** `POST /counterparties/{id}/group/suggest` — advisory parent-group + sibling fuzzy match, human still tags) |
| origination-service | 8083 | `/origination` | Applications, document classification, financial spreading with provenance, **proposed facilities + sublimits + interchangeability**, collaterals, **specialised deal structures** (`/api/applications/{ref}/structure` — group / joint / dual-obligor / syndication / FI ICR / copy-from), **GenAI document intelligence** (`/api/doc-intel` — extract / confirm / normalise / translate / checks), **collateral intelligence** (`/api/collateral-intel` — type-aware extraction per PROPERTY/VEHICLE/INSURANCE/TITLE/BOND/PG with missing-mandatory list, LTV-driven revaluation alerts with human apply-gate, and charge-Excel CSV export) |
...
| decision-service | 8085 | `/decision` | DoA approval, **client planning template** `/api/cpt` (AI-drafted, RM-confirmed: client overview · exposure rollup · cross-sell whitespace · 3-scenario wallet sizing · industry headwinds/tailwinds · completeness nudges; deterministic figures quoted verbatim), covenants + **tracking workflow** (`/api/covenants/tracking`) + **covenant intelligence** `/api/covenants/intel` (AI extraction of covenant defs/thresholds from CP free text → human-confirm materialises a real covenant; compliance-certificate assessment with taxonomy-mismatch + deterministic-recompute disagreement detection), credit proposal, **CAD** `/api/cad` (checklist from master, 2-level waiver/deviation, limit-release trigger), **MER** `/api/mer` (deferred docs, renewals, reminders, escalation sweep, DMS feed), **document generation** `/api/docs` (DOC_TEMPLATE / TNC clauses + clause surgery + human-confirm lock), **AI narrative commentary** `/api/commentary` (grounded section drafters, advisory), **group decisioning** `/api/decisions/groups/{ref}` (advisory rollup insights + combined credit proposal across every member, member figures unchanged) |

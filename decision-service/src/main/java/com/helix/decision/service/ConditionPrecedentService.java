...
public class ConditionPrecedentService {
    ...
    /**
     * Nudges the desk about still-open mandatory CPs on every facility of a deal — one
     * notification per facility with open blockers. A pure read + enqueue (no CP mutation);
     * idempotent per (facility, open-count) so repeated sweeps don't spam.
     */
    @Transactional
    public int nudgeOpen(String applicationReference, String actor) {
        List<String> facilities = repo.findByApplicationReferenceOrderByIdAsc(applicationReference).stream()
                .map(ConditionPrecedent::getFacilityRef).distinct().toList();
        int nudged = 0;
        for (String facilityRef : facilities) {
            CpGateResult gate = gate(applicationReference, facilityRef);
            if (gate.blockers() == null || gate.blockers().isEmpty()) continue;
            String codes = gate.blockers().stream().map(CpBlocker::code).collect(java.util.stream.Collectors.joining(", "));
            try {
                notifications.enqueue(new NotificationService.Enqueue("CP_NUDGE", "CP_NUDGE",
                        "Application", applicationReference,
                        "cp:" + facilityRef + ":open:" + gate.mandatoryOpen(), null,
                        Map.of("borrower", applicationReference, "facilityRef", facilityRef,
                                "openCount", gate.mandatoryOpen(), "codes", codes), null), actor);
                nudged++;
            } catch (Exception e) {
                // notification failures never break the nudge sweep
            }
        }
        audit.engine("CP_NUDGE_SWEEP", "Application", applicationReference,
                "Nudged %d facility(ies) with open CPs".formatted(nudged), Map.of("nudged", nudged));
        return nudged;
    }
    ...
    /**
     * Materialises the structured conditions of sanction captured on an APPROVE /
     * CONDITIONAL_APPROVE decision into the CP register (source {@code SANCTION}) so the
     * existing pre-disbursement gate enforces them. Each spec pins to its {@code facilityRef};
     * a blank facilityRef fans the condition out across every facility on the deal.
     *
     * <p>Idempotent per (facilityRef, code, source=SANCTION) so a re-decide / retry never
     * duplicates. Resilient: if the deal envelope is unreachable AND a spec carries no
     * explicit facilityRef, the condition is NOT dropped silently — it is recorded on a
     * {@code SANCTION_CP_MATERIALIZATION_DEFERRED} audit event (with its code/title in the
     * payload) for manual re-add, and the decision itself is never failed.</p>
     */
    @Transactional
    public List<ConditionPrecedent> materializeFromDecision(String applicationReference,
                                                            List<com.helix.decision.dto.Dtos.ConditionSpec> specs,
                                                            String actor) {
        if (specs == null || specs.isEmpty()) return List.of();
        DealEnvelopeDto env = upstream.envelopeOrNull(applicationReference);
        List<String> dealFacilities = new ArrayList<>();
        if (env != null && env.facilities() != null) {
            for (FacilityViewDto f : env.facilities()) dealFacilities.add(f.reference());
        }
        List<ConditionPrecedent> created = new ArrayList<>();
        List<Map<String, Object>> deferred = new ArrayList<>();
        int n = 0;
        for (com.helix.decision.dto.Dtos.ConditionSpec spec : specs) {
            n++;
            String code = spec.code() == null || spec.code().isBlank() ? "COS-" + n : spec.code();
            String title = spec.title() == null || spec.title().isBlank() ? "Condition of sanction" : spec.title();
            boolean mandatory = spec.mandatory() == null || spec.mandatory();
            List<String> targets;
            if (spec.facilityRef() != null && !spec.facilityRef().isBlank()) {
                targets = List.of(spec.facilityRef());
            } else if (!dealFacilities.isEmpty()) {
                targets = dealFacilities;   // fan out to every facility
            } else {
                deferred.add(Map.of("code", code, "title", title, "mandatory", mandatory));
                continue;
            }
            for (String facRef : targets) {
                boolean exists = repo.findByApplicationReferenceAndFacilityRefOrderByIdAsc(applicationReference, facRef)
                        .stream().anyMatch(cp -> "SANCTION".equals(cp.getSource()) && code.equalsIgnoreCase(cp.getCode()));
                if (exists) continue;   // idempotent
                ConditionPrecedent cp = new ConditionPrecedent();
                cp.setApplicationReference(applicationReference);
                cp.setFacilityRef(facRef);
                cp.setCode(code);
                cp.setTitle(title);
                cp.setDescription(spec.description());
                cp.setMandatory(mandatory);
                cp.setSource("SANCTION");
                created.add(repo.save(cp));
            }
        }
        if (!created.isEmpty()) {
            audit.engine("SANCTION_CP_MATERIALIZED", "Application", applicationReference,
                    "Materialised %d condition(s) of sanction into the CP register".formatted(created.size()),
                    Map.of("count", created.size(),
                            "codes", created.stream().map(ConditionPrecedent::getCode).distinct().toList()));
        }
        if (!deferred.isEmpty()) {
            audit.engine("SANCTION_CP_MATERIALIZATION_DEFERRED", "Application", applicationReference,
                    "Deferred %d condition(s): deal envelope unavailable and no explicit facilityRef — re-add via /cps"
                            .formatted(deferred.size()),
                    Map.of("deferred", deferred));
        }
        return created;
    }

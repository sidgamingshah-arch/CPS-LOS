  ...
  return (
    <div className="grid">
      <Card title="Financial spreading"
        ...
        right={a && <Badge kind={a.spreadConfirmed ? "ok" : "warn"}>{a.spreadConfirmed ? "CONFIRMED" : "DRAFT"}</Badge>}>
            ...
            <table>
              <thead>
                <tr>
                  <th>Line item</th>
                  {periods.map((p: any) => (
                    <th key={p.periodId} className="num">{p.label}<br /><small className="prov">{p.gaap} · {p.currency}</small></th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={key}>
                      ...
                      {periods.map((p: any) => {
                        ...
                        if (c.derived) {
                          return <td key={p.periodId} className="num" style={{ color: "var(--muted)" }}>{fmt.money(c.value, "")}</td>;
                        }
                        return (
